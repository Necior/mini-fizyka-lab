...
\caption{Wykres zależności między wartością napięcia na rezystorze $R_4$ a wartością natężenia prądu.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Wartości pośrednie obliczeń, które pozwalają wyznaczyć wartość współczynnika kierukowego w zagadnieniu regresji liniowej.}
\end{table}
...
Oznaczmy powyższe pomiary jako $x_i$, $i = 1, \dots, 60$.
...
% sqrt(3)?
Niepewność typu B: $u_x (\text{typ B}) = \Delta x_e = 0.005 \enskip \text{mm}$.

% sqrt(3)?
Niepewność całkowita: $u_x = \sqrt{(u_x (\text{typ A}))^2 + (u_x (\text{typ B}))^2} \approx 0.0051196 \approx 0.0052$.

\begin{itemize}
\item $d = 2.9365$ mm, $u_x = 0.0052$ mm
\item $d = 2.9365 \, (52)$ mm
\item $d = 2.9365 \, (0.0052)$ mm
\end{itemize}
